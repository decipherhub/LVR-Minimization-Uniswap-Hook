...
contract TestDiamondHook is Test, Deployers, GasSnapshot {
    ...
    TestERC20 token1;
    PoolId poolId;
    DiamondHookFutures hook = DiamondHookFutures(
        address(
            uint160(
    ...
    function setUp() public {
        ...
        // Create the pool
        poolKey = PoolKey(Currency.wrap(address(token0)), Currency.wrap(address(token1)), fee, tickSpacing, hook);
        poolId = poolKey.toId();
        uint256 price = 1;
        manager.initialize(poolKey, computeNewSQRTPrice(price), new bytes(0));
    ...
    function testOpeningTotalSupplyZero() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(DiamondHookFutures.TotalSupplyZero.selector));
    ...
    function testSwaps() public {
        ...
        IPoolManager.SwapParams memory params = IPoolManager.SwapParams({
            zeroForOne: true,
            amountSpecified: 10 ** 15,
            sqrtPriceLimitX96: computeNewSQRTPrice(3)
        });
        ...
        swapRouter.swap(poolKey, params, settings, new bytes(0));
        ...
        params = IPoolManager.SwapParams({
            zeroForOne: false,
            amountSpecified: -int256(hedgeRequired1),
            sqrtPriceLimitX96: computeNewSQRTPrice(5)
        });
        ...
        params = IPoolManager.SwapParams({
            zeroForOne: true,
            amountSpecified: 2 ** 80,
            sqrtPriceLimitX96: computeNewSQRTPrice(4)
        });
